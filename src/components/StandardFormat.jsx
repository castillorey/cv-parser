...
function estimateJobHeight(ex) {
  const responsibilities = (ex.responsibilities || []).filter(Boolean);
  const respLines = responsibilities.reduce(
    (n, r) => n + Math.max(1, Math.ceil(r.length / CHARS_PER_LINE)),
    0
  );
  return JOB_BASE_HEIGHT + respLines * JOB_LINE_HEIGHT;
}
...
function splitExperience(experiences) {
  ...
  for (; i < experiences.length; i++) {
    ...
    if (h > PAGE_1_WORK_CAPACITY) break;
